...
cd $PROJECT/ && \
docker build -t dockerfile.jdk8.alpine.34.111 -f ./src/main/dockerfiles/Dockerfile.jdk8.alpine.34.111 ./ &&
docker run -it -p 9090:9090 -p 2020:2020 -e "COYOTE_STAGE=0" dockerfile.jdk8.alpine.34.111
...
./src/main/demoscript/coyote.js --action run --image 0 --stage 1 --test 0
...
[source,java,medium]
.start n docker with 512M (this is production, ressources are shared ;-)
/coyote.js --action run --image 0 --stage 2 --test 0
...
[source,java,medium]
.start n docker with 256M (this is production, ressources are shared ;-)
/coyote.js --action run --image 0 --stage 2 --test 1
